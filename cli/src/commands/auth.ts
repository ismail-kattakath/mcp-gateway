...
export function createAuthCommand(): Command {
  ...
  auth
    ...
    .action(async () => {
      ...
      try {
        ...
        spinner.fail(chalk.red(`Failed to retrieve API key: ${(error as Error).message}`));
        process.exit(1);
      }
    });
  ...
  auth
    .command('enable')
    .description('Enable authentication (set disableAuth: false in auth config)')
    ...
    .action((options: { registry: string }) => {
      ...
      try {
        ...
        console.log(chalk.gray('Restart the gateway for changes to take effect'));
      } catch (error) {
        spinner.fail(chalk.red(`Failed to enable auth: ${(error as Error).message}`));
        process.exit(1);
      }
    });
  ...
  auth
    .command('disable')
    .description('Disable authentication (set disableAuth: true in auth config)')
    .requiredOption('-r, --registry <path>', 'Path to registry.json')
    .action((options: { registry: string }) => {
      ...
      try {
        ...
        console.log(chalk.gray('Anyone with network access can call APIs without authentication'));
        console.log(chalk.gray(`\nConfig file: ${getAuthConfigPath(registryPath)}`));
        console.log(chalk.gray('Restart the gateway for changes to take effect'));
      } catch (error) {
        spinner.fail(chalk.red(`Failed to disable auth: ${(error as Error).message}`));
        process.exit(1);
      }
    });
  ...
  auth
    ...
    .requiredOption('-r, --registry <path>', 'Path to registry.json')
    .action((action: string = 'list', ip: string | undefined, options: { registry: string }) => {
      const registryPath = resolve(options.registry);
      const config = loadAuthConfig(registryPath);
      ...
      switch (action) {
        case 'list':
          if (allowedIPs.length === 0) {
            console.log(chalk.yellow('No IP allowlist configured (all IPs allowed)'));
          } else {
        ...
        case 'clear':
          if (allowedIPs.length === 0) {
            ...
            config.allowedIPs = [];
            saveAuthConfig(config, registryPath);
            console.log(chalk.green('Cleared IP allowlist (all IPs now allowed)'));
            console.log(chalk.gray('Restart the gateway for changes to take effect'));
          }
          break;
