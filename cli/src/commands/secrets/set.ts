...
export default class SecretsSet extends BaseCommand {
  static description = 'Store a secret in the system keychain (requires keytar)';
  ...
  async run(): Promise<void> {
    ...
    if (!/^[A-Z][A-Z0-9_]*$/.test(args.key)) {
      this.error(
        chalk.red('Invalid key format. Keys must be uppercase alphanumeric with underscores.')
      );
    }
    ...
    try {
      ...
      this.error(chalk.red(`Failed to store secret: ${(error as Error).message}`));
    }
  }
}
