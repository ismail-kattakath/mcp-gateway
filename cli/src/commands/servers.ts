...
export function createServersCommand(client: ApiClient): Command {
  const servers = new Command("servers").description("Manage MCP servers");
  ...
  servers
    ...
    .action(async () => {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to list servers: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
  ...
  servers
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to get server: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
  ...
  servers
    ...
    .requiredOption(
      "-s, --source <type>",
      "Server source: pkg, git, container, remote, local",
    )
    ...
    .option(
      "--lifecycle <type>",
      "Lifecycle: persistent, on-demand (default: on-demand)",
      "on-demand",
    )
    ...
    .action(
      async (
        ...
        try {
          ...
          spinner.succeed(
            chalk.green(`Server ${chalk.bold(name)} created successfully`),
          );
        } catch (error) {
          spinner.fail(
            chalk.red(`Failed to create server: ${(error as Error).message}`),
          );
          process.exit(1);
        }
      },
  ...
  servers
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to start server: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
  ...
  servers
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to stop server: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
  ...
  servers
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to restart server: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
  ...
  servers
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to enable server: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
  ...
  servers
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to disable server: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
  ...
  servers
    ...
    .action(async (name: string, options: { force?: boolean }) => {
      if (!options.force) {
        console.log(
          chalk.yellow(
            `⚠️  Are you sure you want to delete ${chalk.bold(name)}?`,
          ),
        );
      ...
      try {
        ...
        spinner.fail(
          chalk.red(`Failed to delete server: ${(error as Error).message}`),
        );
        process.exit(1);
      }
    });
