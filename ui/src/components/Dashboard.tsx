...
function Dashboard(): JSX.Element {
  ...
  if (isLoading)
    return (
      <div className="flex items-center justify-center h-64 text-gray-400">Loading status...</div>
    );
  if (error) {
    return (
  ...
  const stoppedCount = entries.filter(
    ([_, s]) => s.state === 'stopped' || s.state === 'not_started'
  ).length;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatusCard
          title="Uptime (s)"
          value={Math.round(status?.gateway?.uptime || 0)}
          icon={Activity}
          color="text-blue-500"
        />
      </div>
