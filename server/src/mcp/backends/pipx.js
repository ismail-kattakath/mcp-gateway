/**
 * PIPX Backend Spawner
 *
 * Manages lifecycle of PIPX-based MCP backends (Python packages via pipx)
 */
...
export class PipxBackend extends EventEmitter {
  ...
  async spawn() {
    ...
    try {
      ...
      logger.info(`Spawning PIPX backend: ${this.backendId}`, {
        package: packageSpec,
        args: args.slice(2)
      });
...
export function createPipxBackend(backendId, config) {
  return new PipxBackend(backendId, config);
}
