/**
 * Local Backend Spawner
 *
 * Executes local scripts/binaries as MCP backends
 */
...
export class LocalBackend extends EventEmitter {
  ...
  async spawn() {
    ...
    try {
      ...
      const scriptPath = this.resolvePath(install.path);
      ...
      let command = runtime?.command || 'node';
      let args = [scriptPath];
      ...
      // If path is executable, run it directly
      if (install.executable) {
        command = scriptPath;
        args = runtime?.args || [];
      }

      logger.info(`Spawning local backend: ${this.backendId}`, {
        command,
        path: scriptPath,
        args
      });
      this.addLog('info', 'Starting local script', { command, path: scriptPath });
  ...
  getStatus() {
    return {
      ...
      path: this.config.install.path,
      command: this.config.runtime?.command || 'node'
    };
  }
...
export function createLocalBackend(backendId, config) {
  return new LocalBackend(backendId, config);
}
