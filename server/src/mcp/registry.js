 ...
 */
function resolveEnvVars(value, context = {}) {
  if (typeof value !== 'string') return value;

  return value.replace(/\$\{([^}]+)\}/g, (full, varName) => {
...
function resolveEnvVarsRecursive(obj, context = {}) {
  ...
  const out = {};
  for (const [k, v] of Object.entries(obj)) out[k] = resolveEnvVarsRecursive(v, context);
  return out;
}
...
function applyDefaults(registry) {
  for (const [name, server] of Object.entries(registry.servers || {})) {
    if (server.lifecycle === undefined) server.lifecycle = DEFAULTS.lifecycle;
    if (server.enabled === undefined) server.enabled = DEFAULTS.enabled;
    if (server.timeout === undefined) server.timeout = DEFAULTS.timeout;
  }
  return registry;
}
...
async function loadRegistry(filePath) {
  ...
  const resolved = resolveEnvVarsRecursive(registry);
  ...
  watcher.on('change', async (changedPath) => {
    ...
    try {
      ...
      for (const cb of watchCallbacks) {
        try { await cb(newRegistry, oldRegistry); }
        catch (error) { logger.error('Registry watch callback failed', { error: error.message }); }
      }
    } catch (error) {
      logger.error('Failed to reload registry, keeping old version', { error: error.message });
    }
  });

  watcher.on('error', (error) => logger.error('Registry watcher error', { error: error.message }));
}
