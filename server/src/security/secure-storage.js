...
try {
  ...
  logger.info('keytar not available, will use encrypted file storage', {
    reason: error.code === 'MODULE_NOT_FOUND' ? 'not installed' : error.message
  });
}
...
function deriveEncryptionKey(salt) {
  const machineIdValue = machineId.machineIdSync({ original: true });
 ...
 * @returns {Buffer} Encrypted data format: [salt(32)][iv(16)][tag(16)][ciphertext]
 */
...
async function storeInKeychain(secret) {
  ...
  try {
    ...
    logger.warn('Failed to store in keychain, will use encrypted file', { error: error.message });
    return false;
  }
}
 ...
 * @returns {string|null} Secret or null if not found
 */
async function retrieveFromKeychain() {
  ...
  try {
    ...
    logger.warn('Failed to read from keychain', { error: error.message });
    return null;
  }
}
...
async function deleteFromKeychain() {
  ...
  try {
    ...
    logger.warn('Failed to delete from keychain', { error: error.message });
    return false;
  }
}
 ...
 */
async function storeInEncryptedFile(secret) {
  try {
    ...
    logger.error('Failed to store in encrypted file', { error: error.message, stack: error.stack });
    return false;
  }
}
 ...
 * @returns {string|null} Secret or null if not found/corrupted
 */
async function retrieveFromEncryptedFile() {
  try {
    ...
    if (error.code !== 'ENOENT') {
      logger.warn('Failed to read encrypted file (may be corrupted)', { error: error.message });
    }
    return null;
  }
}
...
async function deleteEncryptedFile() {
  try {
    ...
    if (error.code !== 'ENOENT') {
      logger.warn('Failed to delete encrypted file', { error: error.message });
    }
    return false;
  }
}
...
export async function storeSecret(secret) {
  ...
  if (keychainSuccess) return true;
 ...
 * @param {string} oldFilePath - Path to old cleartext file
 */
export async function migrateFromCleartext(oldFilePath) {
  try {
    ...
    if (error.code !== 'ENOENT') {
      logger.warn('Failed to migrate from cleartext', { error: error.message });
    }
  }
  return null;
}
